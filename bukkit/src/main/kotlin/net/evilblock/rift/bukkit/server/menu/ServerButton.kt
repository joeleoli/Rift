...
open class ServerButton(protected val server: Server) : Button() {
    ...
    override fun getDescription(player: Player): List<String> {
        ...
        description.add("")
        description.add("${ChatColor.GRAY}Slots: ${ChatColor.YELLOW}${NumberUtils.format(server.slots)}")
        ...
        if (server.isOnline()) {
            ...
            description.add("${ChatColor.GRAY}Players: ${ChatColor.GREEN}${ChatColor.BOLD}${NumberUtils.format(server.playerCount)}${ChatColor.GRAY}/${ChatColor.GREEN}${NumberUtils.format(server.slots)}")
        }
